...
write_runbook() {
...

  {
    ...
    printf 'if [ "$ADOPT" = "1" ] && [ "$private_network_client" = yes ] && [ "$private_network_status" != ready ]; then append_missing tailscale_connection; fi\n'
    ...
    printf '    *" tailscale "*|*" ssh "*|*" ssh_listener "*|*" tmux "*) return 0 ;;\n'
    printf '    *) return 1 ;;\n'
    printf '  esac\n'
    printf '}\n'
    printf 'next_action() {\n'
    ...
    printf '  fi\n'
    printf '  case " $missing " in\n'
    printf '    *" tailscale_connection "*) printf "authenticate Tailscale or run tailscale up, then rerun %%s" "$0"; return ;;\n'
    printf '  esac\n'
    printf '  case " $missing " in\n'
