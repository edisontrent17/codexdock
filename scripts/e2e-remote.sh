...
write_runbook() {
...

  {
    ...
    printf '}\n'
    printf 'private_network_ip() {\n'
    printf '  if ! command -v tailscale >/dev/null 2>&1; then printf none; return; fi\n'
    printf '  ip=$(tailscale ip -4 2>/dev/null | sed -n '"'"'1p'"'"')\n'
    printf '  if [ -n "$ip" ]; then printf "%%s" "$ip"; else printf none; fi\n'
    printf '}\n'
    printf 'private_network_status() {\n'
    printf '  if ! command -v tailscale >/dev/null 2>&1; then printf missing; return; fi\n'
    printf '  ip=$(private_network_ip)\n'
    printf '  if [ "$ip" != none ]; then printf ready; else printf not_connected; fi\n'
    printf '}\n'
    printf 'no_new_privileges_status() {\n'
    printf '  if [ ! -r /proc/self/status ]; then printf unknown; return; fi\n'
    printf '  value=$(awk '"'"'/^NoNewPrivs:/ {print $2; exit}'"'"' /proc/self/status 2>/dev/null || true)\n'
    printf '  if [ -z "$value" ]; then printf unknown; elif [ "$value" = "1" ]; then printf yes; else printf no; fi\n'
    printf '}\n'
    printf 'sudo_config_owner_uid() {\n'
    printf '  if [ ! -e /etc/sudo.conf ]; then printf missing; return; fi\n'
    printf '  if command -v stat >/dev/null 2>&1; then\n'
    printf '    owner=$(stat -c %%u /etc/sudo.conf 2>/dev/null || true)\n'
    printf '    if [ -n "$owner" ]; then printf "%%s" "$owner"; else printf unknown; fi\n'
    printf '  else\n'
    printf '    printf unknown\n'
    printf '  fi\n'
    printf '}\n'
    printf 'prepare_privilege_status() {\n'
    printf '  if ! command -v sudo >/dev/null 2>&1; then printf missing; return; fi\n'
    printf '  if [ "$(no_new_privileges_status)" = yes ]; then printf no_new_privileges; return; fi\n'
    printf '  owner=$(sudo_config_owner_uid)\n'
    printf '  case "$owner" in\n'
    printf '    0|unknown|missing) ;;\n'
    printf '    *) printf invalid_sudo_config; return ;;\n'
    printf '  esac\n'
    printf '  printf likely_available\n'
    printf '}\n'
    ...
    printf 'private_network_status=$(private_network_status)\n'
    printf 'private_network_ip=$(private_network_ip)\n'
    printf 'prepare_privilege=$(prepare_privilege_status)\n'
    printf 'no_new_privileges=$(no_new_privileges_status)\n'
    printf 'sudo_config_owner_uid=$(sudo_config_owner_uid)\n'
    ...
    printf 'if [ "$ADOPT" = "1" ] && [ "$private_network_client" != yes ]; then append_missing private_network_client; fi\n'
    printf 'if [ "$ADOPT" = "1" ] && [ "$private_network_client" = yes ] && [ "$private_network_status" != ready ]; then append_missing private_network_enrollment; fi\n'
    ...
    printf 'prepare_required() {\n'
    printf '  case " $missing " in\n'
    printf '    *" private_network_client "*|*" ssh "*|*" ssh_listener "*|*" tmux "*) return 0 ;;\n'
    printf '    *) return 1 ;;\n'
    printf '  esac\n'
    printf '}\n'
    printf 'next_action() {\n'
    ...
    printf '  esac\n'
    printf '  if prepare_required; then\n'
    printf '    case "$prepare_privilege" in\n'
    printf '      no_new_privileges|invalid_sudo_config|missing) printf "run %%s to inspect, run %%s from a normal WSL terminal with working sudo, then rerun %%s" "$PREPARE_PLAN" "$PREPARE" "$0"; return ;;\n'
    printf '    esac\n'
    printf '  fi\n'
    printf '  case " $missing " in\n'
    printf '    *" private_network_enrollment "*) printf "complete CodexDock private network enrollment, then rerun %%s" "$0"; return ;;\n'
    printf '  esac\n'
    printf '  case " $missing " in\n'
    ...
    printf 'private_network_status=$private_network_status\n'
    printf 'private_network_ip=$private_network_ip\n'
    printf 'prepare_privilege=$prepare_privilege\n'
    printf 'no_new_privileges=$no_new_privileges\n'
    printf 'sudo_config_owner_uid=$sudo_config_owner_uid\n'
